	...
	if spindashPart and spindashPart:IsA("BasePart") and not currentSpindashModel then
		...
		if ok and objects and #objects > 0 then
			...
			if inserted then
				...
				for _, descendant in ipairs(spindashPart:GetDescendants()) do
...
local function monitorSpindashMesh()
	...
	meshReplacementConnection = RunService.Heartbeat:Connect(function()
		if isScriptActive and tick() % 0.5 < 0.1 then
			checkAndReplace()
		end
	end)
end
...
local function setupSonicViewport()
	...
	local function replaceViewportModel()
		...
		if viewportRootPart then
			newModel:PivotTo(viewportRootPart.CFrame * CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(0)))
	...
	local viewportModelDescendantConnection = nil
	viewportModelDescendantConnection = viewportModel.DescendantAdded:Connect(function() 
		if viewportModelDescendantConnection then
			viewportModelDescendantConnection:Disconnect()
			viewportModelDescendantConnection = nil
		end
		task.spawn(function()
			task.wait(0.5)
			replaceViewportModel()
...
local lastSonicCheckTime = 0
local sonicCheckInterval = 0.5

local function getPlayersFolder()
	if not playersFolderCache or not playersFolderCache.Parent then
		playersFolderCache = workspace:FindFirstChild("Players")
	end
	return playersFolderCache
end
...
local function onHeartbeat()
	local currentTime = tick()
	if currentTime - lastSonicCheckTime >= sonicCheckInterval then
		lastSonicCheckTime = currentTime
