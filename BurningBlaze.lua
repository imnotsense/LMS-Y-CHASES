		...
		local function replaceViewportModel()
			...
			for _, v in ipairs(newModel:GetDescendants()) do
				...
				elseif v:IsA("ParticleEmitter") then
					-- Reducción drástica de partículas para el viewport
					v.Rate = math.clamp(v.Rate * 0.1, 0, 5)
				end
			end
...
local function setupCharacter(char)
	...
	for _, v in ipairs(mdl:GetDescendants()) do
		...
		elseif v:IsA("ParticleEmitter") then
			...
			-- Reducimos la cantidad de partículas emitidas al 30% del original, 
			-- limitándolo a un máximo de 25 por emisor.
			v.Rate = math.clamp(v.Rate * 0.3, 0, 25)
		end
	end
		...
		if primaryPart then
			for _, v in ipairs(spindashMdl:GetDescendants()) do
				if v:IsA("BasePart") then
					...
					v.Anchored = true
				elseif v:IsA("ParticleEmitter") then
					-- Optimización de partículas también para el spindash
					v.Rate = math.clamp(v.Rate * 0.3, 0, 30)
				end
			end
